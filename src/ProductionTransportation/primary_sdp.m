function [ f_opt,x,z_k,q,Q,lambda_k,CPUTime ] = primary_sdp( ZIGMA,mu,A,b,gamma1,gamma2,c,d,alpha_k,beta_k,K,m,n)
...
cvx_begin sdp
    ...
    variable Q(data_size,data_size) symmetric;
    variable lambda_k(K, l);

    %%
    minimize(    s + sum(sum(  (gamma2*eye(data_size)).*Q  )) + sqrt(gamma1)*norm(q,2)    );
    %%
    subject to
