function [ f_opt,x,z_k,q,Q,lambda_k,u_k,CPUTime ] = pca_upperbound( ZIGMA,mu,A,b,gamma1,gamma2,c,d,alpha_k,beta_k,K,m,n,B,m1)
...
cvx_begin sdp
    ...
    for k = 1:K
        [ s-c'*x-beta_k(k)-lambda_k(k,:)*(b-A*mu) - alpha_k(k)*z_k(k,:)*mu     1/2*u_k(k, :) ; 1/2*u_k(k, :)'  Q] >= 0 ;
        q + A1Complete'*(A'*lambda_k(k,:)'-alpha_k(k)*z_k(k,:)') == B*u_k(k, :)';
